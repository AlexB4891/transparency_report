...
1.  Active in 2014: Firms that have fille C.I.T & positive income
2.  Has filled at least once the APS in 2012-2014
...
global_pre_2 <- tabla %>% 
  filter(anio_fiscal <=2014) %>% 
  inner_join(core_sample) %>% 
  mutate(non_cero_income = total_income > 0,
         cero_income = total_income == 0)

summ_row <- bind_cols(
  global_pre_2 %>% 
    group_by(non_cero_income) %>% 
    summarise(positive = n_distinct(identificacion_informante_anon)) %>% 
    filter(non_cero_income) %>% 
...
stats_aps <- tabla %>% 
  ...
  mutate(sin_aps = replace_na(sin_aps,0),
         ...
         post = factor(post,c(0,1),c("Pre reform","Post reform"))) %>% 
...
Let's take the following definition of change:
 ...
 -  If ythe Assigned group in 2014 is "T-Maj" & the effective participation in tax haven post reform  is  < 50
 -  If ythe Assigned group in 2014 is "T-Min" & the effective participation in tax haven post reform  is  >= 50 
 -  If ythe Assigned group in 2014 is "C-Maj" & the effective participation in tax haven post reform  is  > 5 
 - If ythe Assigned group in 2014 is "C-Min" & the effective participation in tax haven post reform  is  > 5 
...
compar2 <- tabla_core_p %>% 
  ...
  mutate(cambio = case_when(
    ...
    group_assign_2014 == "T-Min" & porcentaje_aps_pff >= 50 ~ 1,
    group_assign_2014 == "C-Maj" & porcentaje_aps_pff > 5 ~ 1,
    group_assign_2014 == "C-Min" & porcentaje_aps_pff > 5 ~ 1,
    TRUE ~ 0
  ))
...
compar2 %>% 
  ...
  mutate(
    ...
    dummy_change_5ff = as.numeric(diferencia_pff > 5),
    dummy_change_0ff = as.numeric(porcentaje_aps_pff == 0 & porcentaje_aps_pff_2014 >0)
  ) %>% 
  group_by(anio_fiscal,group_assign_2014) %>% 
...
compar2 %>%
  ...
  mutate(
    predicted_tasa = case_when(
      ...
      TRUE ~ 0.03),
    indicador = as.numeric(predicted_tasa == diferencia_tasa)
  ) %>% 
  group_by(anio_fiscal,group_assign_2014) %>% 
