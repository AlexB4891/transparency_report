  ...
  imap(~{
    ...
    estados <- compar %>% 
      mutate(estado_new = as.numeric(is.na(anio_fiscal_post)),
             ...
             condicion_1 = porcentaje_aps_pff_2014 > 0.4995 & between(porcentaje_aps_pff_post,0.00005, 0.4995),
             # Reducen por debajo del umbral al 0:
             condicion_2 = porcentaje_aps_pff_2014 > 0.4995 & porcentaje_aps_pff_post < 0.00005 ,
             # Reducen por encima del umbral:
             condicion_3 = porcentaje_aps_pff_2014 > 0.4995 & porcentaje_aps_pff_post > 0.4995 & porcentaje_aps_pff_2014 > porcentaje_aps_pff_post,
             # Mantienen pff:
             condicion_4 = porcentaje_aps_pff_2014 > 0.4995 & between(porcentaje_aps_pff_2014 - porcentaje_aps_pff_post,-0.00005,0.00005),
             # Incrementan los pff:
             condicion_5 = porcentaje_aps_pff_2014 > 0.4995 & (porcentaje_aps_pff_post-  porcentaje_aps_pff_2014) > 0.00005 ,
...
preparar_gt <- function(condicion){
...
```{r}
preparar_gt("condicion_4")

```

##### T-Maj: Firms that increase same tax haven participation

```{r}
preparar_gt("condicion_5")
...
-   We compare the results each of the fiscal year in the post period and evaluate the cahnges in their APS and CIT fills
-   We consider the following cases for this group:
    -   Reduce the tax haven participation below 5% but above 0
    -   Reduce the tax haven participation to 0
    -   Increase the tax haven participation above 5% but below 50%
    -   Keep the same tax haven participation
    -   Increase the tax haven participation above 5% and above 50%
  ...
  imap(~{
    ...
    estados <- compar %>% 
      mutate(estado_new = as.numeric(is.na(anio_fiscal_post)),
             ...
             condicion_1 = porcentaje_aps_pff_2014 < 0.05 & between(porcentaje_aps_pff_post,0.00005, 0.04995) & porcentaje_aps_pff_2014 > porcentaje_aps_pff_post,
             # Reducen por debajo del umbral pff para C al 0:
             condicion_2 = porcentaje_aps_pff_2014 < 0.05 & porcentaje_aps_pff_post < 0.00005 ,
             # Incrementan por encima del 5%  pero por debajo del 50%:
             condicion_3 = porcentaje_aps_pff_2014 < 0.05 & between(porcentaje_aps_pff_post,0.05005, 0.4995),
             # Mantienen pff:
             condicion_4 = porcentaje_aps_pff_2014 < 0.05 & between(porcentaje_aps_pff_2014 - porcentaje_aps_pff_post,-0.00005,0.00005),
             # Incrementan los pff por encima del 50%:
             condicion_5 = porcentaje_aps_pff_2014 < 0.05 & porcentaje_aps_pff_post >  0.4995,
             # Dejan de declarar APS:
             condicion_6 = as.numeric(estado_new == 1),
             across(c(condicion_1,condicion_2,condicion_3,condicion_4),as.numeric),
             anio_resumen = .y
...
##### T-Min: Firms that increase the tax haven participation above 5% and above 50%
...
### Distribution of the changes in tax haven participation

```{r}
aps_core_2014 <- tabla_core_p %>% 
  filter(anio_fiscal == 2014,group_assign %in% c("T-Min","T-Maj"))
...
comparativo <- tabla_core_p %>%
  ...
  imap(~{
    ...
    compar <- compar %>%
      mutate(
        diferencia_pff = 
          if_else(!is.na(porcentaje_aps_pff_2014) & !is.na(porcentaje_aps_pff_post),
                  porcentaje_aps_pff_2014 - porcentaje_aps_pff_post,
                  NA_real_),
        anio_resumen = .y) %>% 
      select(identificacion_informante_anon,diferencia_pff,anio_resumen,group_assign_2014)
...
comparativo %>% 
  ggplot() +
  geom_histogram(aes(x = diferencia_pff, fill = group_assign_2014)) +
  facet_wrap(facets =group_assign_2014 ~ anio_resumen, scales = "free_y") +
  theme_minimal() +
  theme(legend.position = "none") +
  labs(title = "Distribution of the change in tax haven aprticipation",
       subtitle = "Each year is compared to 2014",
       y = "Frecuency",
       x = "Diference between tax haven participation")
